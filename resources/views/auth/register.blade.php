<x-guest-layout>
    <div class="d-flex flex-column gap-2 mb-3">
        <a href="{{ route('login.google') }}" class="btn btn-light border rounded-xl py-2 w-100 d-flex align-items-center justify-content-center gap-2"
            style="background:#fff; border-color:#f3e7ea;">
            <span class="material-symbols-outlined" style="font-size:20px;">account_circle</span>
            التسجيل عبر Google
        </a>
        <a href="{{ route('login.phone') }}" class="btn btn-outline-secondary rounded-xl py-2 w-100 d-flex align-items-center justify-content-center gap-2"
    ...
    <div class="text-center text-muted small mb-2">أو أكمل البيانات التالية</div>
    ...
    <form method="POST" action="{{ route('register') }}" class="d-flex flex-column gap-3">
        ...
        <div class="p-3 rounded-3" style="background: #fff5f7; border:1px solid #f3e7ea;">
            <div class="small fw-bold mb-1" style="color:#9a4c5f;">نوع الحساب</div>
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 small text-muted">
                <span>يتم إنشاء الحساب كـ <strong>مستخدم</strong> بشكل افتراضي.</span>
                <span>صلاحيات الأدمن تُمنح من لوحة التحكم فقط.</span>
            </div>
        </div>

        <button type="submit" class="auth-btn mt-1">إنشاء الحساب</button>
